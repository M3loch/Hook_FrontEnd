...
class User {
	...
	async _init() {
		if (this._hasToken) {
			...
			response.isExist ? this._setUser(response.user) : this.logOut();
	...
	async logIn(phoneNumber, pass) {
		...
		response.isExist && this._setUser(response.user);
		response.isExist && this._setToken(response.user.access_token);
		this._setIsAuth(response.isExist);
		return response.isExist;
	}
	...
	async register(phoneNumber, firstName, familyName, password) {
		...
		response ? await this.logIn(phoneNumber, password) : null;
	}

	logOut() {
		console.log("выход");
		this._setIsAuth(false);
		this._hasToken() ? this._clearToken() : null;
	...
	async createShop(shopName) {
		...
		newShopList ? (this.shops = newShopList) : null;
		return newShopList && true;
	}

	async updateUser(updateRequest) {
		const newUserInfo = await UserService.updateUser(
			this.userID,
			this.accessToken,
			updateRequest
		);
