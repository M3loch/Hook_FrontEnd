...
function EmployeeCard({employee, setEmployeeList}){
    ...
    async function deleteEmployee(){
        ...
        const filtredEmployeeList = currentEmployees.filter((emp)=>{return emp.employee_id != employee.employee_id})
        const request = {}
        for (const employee of filtredEmployeeList){
            request[`${employee.employee_id}`] = employee.employee_role
        }
        console.log(request)
        const newShopInfo = await shop.updateShop(user.accessToken, {employees: request})
    ...
    return (
        <div className="block-value"> 
            ...
            <div className="employee-table-column">
