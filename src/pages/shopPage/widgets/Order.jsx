...
function Order({ order }) {
	...
	const [nextStageButtonInnerText, setNextStageButtonInnerText] = useState(
		order.isLastStage() ? "Закрыть заказ" : "Следующая стадия"
	);
	...
	return (
		<div className={"order"}>
			...
			<div className="order-header">
				<div className="stage-name">{order.stage}</div>
				<Button
			...
			<div className="timer">{order.timeInterface}</div>
			<div className="control">
				...
				<Button
					...
					innerText={
						<img
							src={arrow}
							className={compressed ? "arrow-down" : "arrow-up"}
						/>
					}
				...
				<div className="order-info">
					<div className="setting-container">
						...
						<SelectField
							...
							callBack={() => {
								order.updateOrder(user.userID, { category_id: category });
							}}
						/>
					</div>
					<div className="setting-container">
						...
						<SelectField
							...
							callBack={() => {
								order.updateOrder(user.userID, { table_id: table });
							}}
						/>
					</div>
					<div className="setting-container">
						...
						<SelectField
							...
							callBack={() => {
								order.updateOrder(user.userID, { discount_id: discount });
							}}
						/>
					</div>
					<div className="setting-container">
						...
						<CheckboxField
							...
							setValue={setIsPaid}
							callBack={() => {
								order.updateOrder(user.userID, { is_paid: isPaid });
							}}
							innerText={"Обновить"}
						/>
					</div>
					<div className="setting-container">
						...
						<Field
							...
							type="number"
							callBack={() => {
								order.updateOrder(user.userID, { strength: strength });
							}}
							innerText={"Обновить"}
						/>
					</div>
					<Field
						...
						placeholder={"comment"}
						callBack={() => {
							order.updateOrder(user.userID, { comment: comment });
						}}
						innerText={"Обновить"}
						textArea={true}
					/>
